<?php

/**
 * @param CI_Model $db
 */
class Harmonogramy_model extends CI_Model {
    private $t_duties;
    private $t_duties_dependencies;
    private $t_duty_week_days;
    private $t_roles;
    private $t_roles_duty_limitis;
    private $t_roles_duty_priorities;
    private $t_schedules;
    private $t_schedules_duties;
    private $t_timetable_history;
    private $t_users;
    private $t_users_duties;
    private $t_users_roles;
    private $t_users_schedules;
    private $t_users_workers;
    private $t_workers;
    private $t_workers_duty_limits;
    
    private $user_id;

    public function  __construct() {
        parent::__construct();
        $this->config->load();

        $this->t_duties = 'duties';
        $this->t_duties_dependencies = 'duties_dependencies';
        $this->t_duty_week_days = 'duty_week_days';
        $this->t_roles = 'roles';
        $this->t_roles_duty_limitis = 'roles_duty_limitis';
        $this->t_roles_duty_priorities = 'roles_duty_priorities';
        $this->t_schedules = 'schedules';
        $this->t_schedules_duties = 'schedules_duties';
        $this->t_timetable_history = 'timetable_history';
        $this->t_users = $this->config->item('db_table_prefix', 'tank_auth') . 'users';
        $this->t_users_duties = 'users_duties';
        $this->t_users_roles = 'users_roles';
        $this->t_users_schedules = 'users_schedules';
        $this->t_users_workers = 'users_workers';
        $this->t_workers = 'workers';
        $this->t_workers_duty_limits = 'workers_duty_limits';

        $this->user_id = $this->session->userdata('user_id');
    }

    public function addUserSchedule($schedule_id) {
        $d = array(
            'user_id' => $this->user_id,
            'schedule_id' => $schedule_id
        );
        $this->db->insert($this->t_users_schedules, $d);
    }

    public function editSchedule($schedule_id) {
        $d = array(
            'name' => $this->input->post('name', true)
        );

        $this->db->trans_start();

        if (0 === $schedule_id) {
            $this->db->insert($this->t_schedules, $d);
            $id = $this->db->insert_id();
            $this->addUserSchedule($id);
        } else {
            $this->db->where('id', $schedule_id)->update($this->t_schedules, $d);
        }

        $this->db->trans_complete();
        if (false === $this->db->trans_status())
            return false;
        return true;
    }

    public function getSchedule($schedule_id) {
        $res = $this->db->select()
                        ->from($this->t_users_schedules . ' AS us')
                        ->join($this->t_schedules . ' AS s', 's.id = us.schedule_id')
                        ->where('us.user_id', $this->user_id)
                        ->where('us.schedule_id', $schedule_id)
                        ->get()->row_array();
        return $res;
    }

    public function deleteSchedule($schedule_id) {
        
    }

    public function scheduleExists($schedule_name, $schedule_id = 0) {
        $res = $this->db->select('s.id')
                        ->from($this->t_users . ' AS u')
                        ->join($this->t_users_schedules . ' AS us', 'u.id = us.user_id')
                        ->join($this->t_schedules . ' AS s', 's.id = us.schedule_id')
                        ->where('us.user_id', $this->user_id)
                        ->where('s.name', $schedule_name)
                        ->get()->result_array();
        if ($schedule_id == @$res[0]['id'] || empty($res))
            return false;
        // istnieje
        return true;
    }

    // LIST
    public function getSchedules($limit, $offset) {
        $res = $this->db->select()
                        ->from($this->t_users_schedules . ' AS us')
                        ->join($this->t_schedules . ' AS s', 's.id = us.schedule_id')
                        ->where('us.user_id', $this->user_id)
                        ->limit($limit, $offset)
                        ->get()->result_array();
        if (!empty($res)) {
            foreach ($res as $key => $s) {
                $res[$key]['duties'] = $this->db->select('d.name')
                                            ->from($this->t_schedules_duties . ' AS sd')
                                            ->join($this->t_duties . ' AS d', 'd.id = sd.duty_id')
                                            ->where('sd.schedule_id', $s['id'])
                                            ->get()->result_array();
                $res[$key]['duties'] = array_map(create_function('$elem', 'return $elem[\'name\'];'), $res[$key]['duties']);
            }
        }
        return $res;
    }

    public function countSchedules() {
        $res = $this->db->select()
                        ->from($this->t_users_schedules . ' AS us')
                        ->join($this->t_schedules . ' AS s', 's.id = us.schedule_id')
                        ->where('us.user_id', $this->user_id)
                        ->get()->num_rows();
        return $res;
    }
}